...
(() => {
  ...
  const STORAGE_KEY = 'crucible-locale';
  ...
  function ensureGoogleTranslateLoaded() {
    ...
    googleReadyPromise = new Promise((resolve) => {
      const includedLanguages = LOCALES.filter((l) => l.code !== 'en')
        .map((l) => l.code)
        .join(',');
      window.__crucibleInitGoogleTranslate = () => {
      ...
      loadScript('https://translate.google.com/translate_a/element.js?cb=__crucibleInitGoogleTranslate');
    });
